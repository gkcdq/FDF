...
void	draw_calcul(t_draw *drw, t_matrice *cor)
{
	...
	while (drw->y < cor->y_max)
	{
		...
		{
			drw->z0 = cor->matrice[drw->y][drw->x] * drw->scale_z;
			drw->x0 = drw->x * 20;
			drw->y0 = (drw->y * 20 - cor->matrice[drw->y][drw->x]);
...
{
	drw->scale_z = ft_scale(cor);
	drw->z1 = cor->matrice[drw->y][drw->x + 1] * drw->scale_z;
	drw->x1 = (drw->x + 1) * 20;
	drw->y1 = (drw->y * 20 - cor->matrice[drw->y][drw->x + 1]);
...
{
	drw->scale_z = ft_scale(cor);
	drw->z1 = cor->matrice[drw->y + 1][drw->x] * drw->scale_z;
	drw->x1 = drw->x * 20;
	drw->y1 = ((drw->y + 1) * 20 - cor->matrice[drw->y + 1][drw->x]);
	draw_isometric(&drw->x1, &drw->y1, drw->z1);
